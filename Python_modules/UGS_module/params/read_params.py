...
try:
    with open(os.path.join(os.getcwd(), 'Python_modules', 'UGS_module','json_data', '../json_data/params_model.json'),
              'r', encoding='utf-8-sig') as f:
        params_data = json.load(f)
except FileNotFoundError:
    sys.exit("Ошибка: Файл c параметрами не найден!")

params = ['intra_month', 'weights_in', 'weights_out', 'functions', 'flows', 'gmt', 'date', 'tech_availability']
...
for key, value in params_data.items():
    if key == 'intra_month':
        ...
        date = value
        variable.append(key)
    elif key == 'tech_availability':
        tech_availability = value
        variable.append(key)
    else:
