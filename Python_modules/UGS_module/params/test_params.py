...
def test_dates(start_date:str, end_date:str):
    """ Проверка формата даты шаблону
    """
    
    if not re.compile(r'01-[0-1][0-9]-20(2[2-9]|3[0-9])', re.UNICODE).search(start_date):
        print(f"start_date {start_date} некорретна")
        return -1
    if not re.compile(r'31-12-20(2[2-9]|3[0-9])', re.UNICODE).search(end_date):
        print(f"end_date {end_date} некорретна")
        return -1
    if not int(end_date[-4:]) - int(start_date[-4:]) >= 2:
        print("Параметр end_date не превышает start_date больше чем два года!")
        return -1
        
    return 0
    

def test_tech_availability(tech_availability: dict):
    """ Проверка коэффициентов техничекой возможности
    """
    
    for phg, value in tech_availability.items():
        if not (0 < value['in'] <= 1 and 0 < value['out'] <= 1):
            print(f"Для ПХГ {phg} коэффициент техничекой возможности должен быть: (0 < x <= 1)")
            return -1
