...
from apps.api.models import PointsOut, UGSNames, PointsIn, UGSSettings
...
from Python_modules.Django_modules import utilities
...
def curve_add_loader(request, html_template, context: dict):
    ...
    if not request.POST.get("pointsnumber") is None:
        if not request.POST.get("UGS") == "":
            ...
            volume_in = request.POST.get("ugs_in")
            volume_out = request.POST.get("ugs_out")
            points_number = request.POST.get("pointsvalue")
            points_number_in = request.POST.get("pointsvalue_in")
            context['name'] = name
            context['capacity'] = capacity
            context['ugs_in'] = volume_in
            context['ugs_out'] = volume_out

            if not points_number is "":
                if not UGSNames.objects.filter(name=name).exists():
                    p = UGSNames(name=name)
                    p.save()
                    settings = UGSSettings(name_id=UGSNames.objects.filter(name=name).values("id").get()["id"],
                                            capacity=capacity, volume_in=volume_in, volume_out=volume_out)
                    settings.save()
    ...
    # На данный момент при наличии в базе значений производится перезапись
        ...
        name = request.POST.get("UGS")
        if not name == "":
            name_id = UGSNames.objects.get(name=name)
            for i in list(range(1, 13)):
                try:
                    ...
                    if 0 <= X_value_out <= 1 and 0 <= Y_value_out <= 1:
                        if i == 1:
                            if PointsOut.objects.filter(name=name_id).exists():
                                PointsOut.objects.filter(name=name_id).delete()
                                p = PointsOut(X=X_value_out, Y=Y_value_out, name=name_id)
                                p.save()
                            else:
                                p = PointsOut(X=X_value_out, Y=Y_value_out, name=name_id)
                                p.save()
                    ...
                    try:
                        ...
                        if 0 <= X_value_in <= 1 and 0 <= Y_value_in <= 1:
                            if i == 1:
                                if PointsIn.objects.filter(name=name_id).exists():
                                    PointsIn.objects.filter(name=name_id).delete()
                                    p = PointsIn(X=X_value_in, Y=Y_value_in, name=name_id)
                                    p.save()
                                else:
                                    p = PointsIn(X=X_value_in, Y=Y_value_in, name=name_id)
                                    p.save()
...
def curve_graph(request, html_template, context: dict):
    ...
    if request.method == "POST":
        ...
        if not request.POST.get("submit_chart") is None:
            ...
            ugs_obj = UGSNames.objects.filter(name=ugs_name)[0].id
            points_value_out = PointsOut.objects.filter(name=ugs_obj).values_list("X", "Y")
            points_value_in = PointsIn.objects.filter(name=ugs_obj).values_list("X", "Y")
            ...
            settings = UGSSettings.objects.filter(name=ugs_obj).values_list("capacity", "volume_in", "volume_out")[0]
            context["capacity"] = round(settings[0], 1)
            context["volume_in"] = round(settings[1], 1)
            context["volume_out"] = round(settings[2], 1)
    return HttpResponse(html_template.render(context, request))
...
def model_create(request, html_template, context: dict):
    ...
    if request.method == "POST":
        ...
        if not request.POST.get("Submit_settings") is None:
            ...
            for UGS in UGSNames_list:
                # Словарь для сохранения значений проверки
                UGS_check = [True, True, True, True, True]

                # Проверка значений параметров
                capacity = UGSSettings.objects.filter(name__name=UGS).values_list()[0][2]
                volume_in = UGSSettings.objects.filter(name__name=UGS).values_list()[0][3]
                volume_out = UGSSettings.objects.filter(name__name=UGS).values_list()[0][4]
                ...
                points_in_x = [point_in[2] for point_in in PointsIn.objects.filter(name__name=UGS).values_list()]
                points_in_y = [point_in[3] for point_in in PointsIn.objects.filter(name__name=UGS).values_list()]
                points_out_x = [point_out[2] for point_out in PointsOut.objects.filter(name__name=UGS).values_list()]
                points_out_y = [point_out[3] for point_out in PointsOut.objects.filter(name__name=UGS).values_list()]
                ...
                weights_in[UGS], temp_check = Try_parse(request.POST.getlist
                                                        ("main_settings_(weights_in_f" + UGS), "list_float")
                if temp_check:
                    UGS_check[1] = False
                    model_complete = False
                ...
                weights_out[UGS], temp_check = Try_parse(request.POST.getlist
                                                         ("main_settings_(weights_out_f" + UGS), "list_float")
                if temp_check:
                    UGS_check[1] = False
                    model_complete = False
...
def model_load(request, html_template, context: dict):
    ...
    if request.method == "POST":
        if not request.POST.get("model_load") is None:
            ...
            # Обратная реформация даты
            context["dates"][0] = json_date_decoder(context["dates"][0])
            ...
            if os.path.exists(os.path.join(os.getcwd(), "Python_modules", "Models_database", model_name + ".json")):
                ...
                return HttpResponseRedirect("/UGS/model_load.html")
            else:
                return HttpResponseRedirect("/UGS/model_load.html")
        else:
            html_template = loader.get_template("home/ugs_settings.html")
...
def load_repairs(request, html_template, context: dict):
    context["UGS_names"] = utilities.load_ugs()
    context["UGS_id"] = list(name_id["id"] for name_id in UGSNames.objects.values("id"))
